...
class Features(nn.Module):
    def __init__(
        ...
        self.last_ch_out = 512 * block.expansion
        self.fpn_channels = [64 * block.expansion, 128 * block.expansion,
    ...
    def _forward_impl(self, x: Tensor):
        ...
        if self.use_fpn:
            # If using FPN, we return the feature maps from all layers
            features = []
            x = self.layer1(x)
...
class ResNet(nn.Module):
    def __init__(self, variant, pretrained=False, use_fpn=False, fpn_out_ch=None):
        super().__init__()
        if variant == 'resnet50':
            self.features = Features(Bottleneck, [3, 4, 6, 3], use_fpn=use_fpn)
        else:
        ...
        if pretrained:
            try:
                self.features.load_state_dict(torch.load(
                    get_pretrained_file_path(f"{variant}.pt")))
