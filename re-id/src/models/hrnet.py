...
from src.utils.split_state_dict import load_state_dict
from src.utils.file_path import PRETRAINED_PATH
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, enable_dim_reduction, dim_reduction_channels, **kwargs):
        ...
        if self.enable_dim_reduction:
            self.out_ch = self.dim_reduction_channels
        else:
            self.out_ch = self.layers_out_channels
    ...
    def load_pretrained(self, model_name: str):
        pretrained_dict = load_state_dict(PRETRAINED_PATH, model_name)
...
def hrnet32(pretrained=False, enable_dim_reduction=True, dim_reduction_channels=256):
    ...
    if pretrained:
        model.load_pretrained("hrnetv2_w32")
    return model
